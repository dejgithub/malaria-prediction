...
class RNNModel(nn.Module):
    def __init__(self, input_size, hidden_size=32, output_size=1, dropout=0.3):
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_size, 16), nn.ReLU(), nn.Linear(16, output_size)
        )
...
def preprocess_data(df, sequence_length=SEQUENCE_LENGTH):
    ...
    Normalization stats are computed from training data only to prevent
...
def train_model(X_train, y_train, X_test, y_test, epochs=100, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({"model_state_dict": model.state_dict(), "val_loss": val_loss}, "models/hybrid_model.pt")
...
def train_and_compare_models(
    X_train, y_train, X_test, y_test, epochs=80, batch_size=32
):
...
def generate_predictions(model, df, case_min, case_max, feature_cols, future_years=15):
    ...
    future_dates = [
        last_date + timedelta(days=30 * (i + 1)) for i in range(future_years * 12)
    ]
...
def create_dashboard_data(historical_df, predictions_df, metrics):
    ...
    high_risk = yearly_pred[yearly_pred["risk_level"].isin(["High", "Very High"])][
        "year"
    ].tolist()
...
def create_charts(historical_df, predictions_df, train_losses, val_losses, model_results=None):
    ...
    plt.plot(
        yearly_pred["year"],
        yearly_pred["predicted_cases"],
        "r--o",
        label="Predicted",
        linewidth=2,
    )
    plt.axvline(
        x=yearly_hist["year"].iloc[-1],
        color="gray",
        linestyle=":",
    ...
    plt.bar(
        np.arange(12) - 0.2, monthly_hist.values, 0.4, label="Historical Avg", alpha=0.7
    )
    plt.bar(
        np.arange(12) + 0.2, monthly_pred.values, 0.4, label="Predicted Avg", alpha=0.7
    )
    ...
    if model_results:
        ...
        bars = plt.bar(
            model_names, mse_values, color=colors[: len(model_names)], edgecolor="white", linewidth=2
        )
        plt.xlabel("Model")
        plt.ylabel("MSE")
...
@app.route("/charts")
def charts_page():
    """Serve the charts/analysis page"""
    try:
...
def charts_page_alt():
    """Serve the charts/analysis page - alternative route"""
    try:
        return send_from_directory(os.getcwd(), "charts.html")
    except:
        return send_from_directory(".", "charts.html")
...
def get_analysis_data():
    """Get analysis data for charts page"""
    try:
        # Load dashboard data
        ...
        # Load predictions monthly for seasonal analysis
        try:
...
def serve_output_file(filename):
    """Serve output files like images"""
...
def upload_file():
    try:
        ...
        # Load best hybrid model for predictions
        print("\nLoading best hybrid model for predictions...")
        model = HybridRNNLSTMGRU(input_size=X_train.shape[2]).to("cpu")
        model.load_state_dict(torch.load("models/hybrid_model.pt", map_location="cpu", weights_only=True)["model_state_dict"])

        # Get hybrid metrics for main dashboard
...
def get_results():
    try:
        ...
        return jsonify({"success": True, "data": data, "metrics": metrics})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
